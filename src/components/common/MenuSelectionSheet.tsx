    ...
    >
      ...
      <motion.div
        ...
        className={`relative w-full max-w-lg bg-neutral-900 border border-neutral-800 ${
          isDesktop ? 'rounded-3xl max-h-[85%]' : 'rounded-t-3xl rounded-b-none border-b-0 max-h-[88%]'
        } overflow-hidden shadow-2xl z-10 text-neutral-100 flex flex-col pointer-events-auto`}
      >
        {/* Top Handle for mobile */}
        {!isDesktop && (
          <div className="w-12 h-1.5 bg-neutral-700 rounded-full mx-auto mt-3 mb-1 flex-shrink-0" />
        )}

        {/* Header */}
        <div className="p-4 sm:p-5 flex items-center justify-between border-b border-neutral-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            {selectedItems.length > 0 && (
              <button
                ...
                className="text-xs text-neutral-400 hover:text-rose-400 px-2.5 py-1 rounded-lg border border-neutral-800 transition-colors flex items-center gap-1 cursor-pointer"
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3.5 no-scrollbar overscroll-contain">
          {selectedItems.length === 0 ? (
        ...
        {selectedItems.length > 0 && (
          <div className="p-4 sm:p-5 border-t border-neutral-800 bg-neutral-950/90 space-y-3.5 flex-shrink-0">
            ...
            <button
              onClick={() => setIsSelectionSheetOpen(false)}
              className="w-full py-3.5 rounded-xl font-bold text-sm text-neutral-950 active:scale-[0.99] transition-all flex items-center justify-center gap-1.5 shadow-lg cursor-pointer"
              style={{ backgroundColor: accentColor }}
            >
