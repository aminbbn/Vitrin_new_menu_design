...
interface RestaurantInfoModalContentProps {
  restaurant: RestaurantData;
  onClose: () => void;
  accentColor: string;
}

const RestaurantInfoModalContent: React.FC<RestaurantInfoModalContentProps> = ({
  restaurant,
  onClose,
  accentColor,
}) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={isDesktop ? { duration: 0.2 } : { type: 'spring', damping: 28, stiffness: 300 }}
        ...
        } overflow-y-auto overscroll-contain no-scrollbar shadow-2xl z-10 text-neutral-100 p-6 space-y-6 pointer-events-auto`}
      >
        {/* Top Handle for mobile */}
        {!isDesktop && (
          <div className="w-12 h-1.5 bg-neutral-700 rounded-full mx-auto -mt-2 mb-2 flex-shrink-0" />
        )}
        ...
        <button
          onClick={onClose}
          className="w-full py-3 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-medium text-sm transition-colors cursor-pointer"
        >
          بستن
        </button>
      </motion.div>
