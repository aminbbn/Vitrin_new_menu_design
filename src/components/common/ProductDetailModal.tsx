...
const ProductDetailOverlayContent: React.FC<ProductDetailOverlayContentProps> = ({
  ...
  // Sheet geometry: exactly ~60% height on mobile & tablet, centered modal on desktop
  const panelGeometryClass = isDesktop
    ? 'w-full max-w-md rounded-3xl border border-neutral-800 max-h-[80vh]'
    : isSimulated
    ? 'w-full h-[60%] max-h-[60%] rounded-t-3xl rounded-b-none border-t border-x border-neutral-800'
    : 'w-full h-[60vh] max-h-[60vh] rounded-t-3xl rounded-b-none border-t border-x border-neutral-800';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Product Food Header Image (Compact height for ~60% sheet) */}
        <div
          className={`relative w-full bg-neutral-950 flex-shrink-0 overflow-hidden ${
            isDesktop ? 'h-44' : 'h-32 sm:h-36'
          }`}
        >
          <img
            src={item.image}
          ...
          <button
            ...
            className="absolute top-2.5 left-2.5 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md text-white flex items-center justify-center hover:bg-black/90 active:scale-95 transition-all border border-white/10 cursor-pointer shadow-lg z-10"
          ...
          <div className="absolute bottom-2.5 right-4 left-4 text-right">
        ...
        <div className="p-4 sm:p-5 space-y-3.5 flex-1 overflow-y-auto overscroll-contain no-scrollbar text-right">
          {/* Price & Availability Banner (Refined, title is strongest) */}
          <div className="flex items-center justify-between p-2.5 sm:p-3 rounded-xl bg-neutral-900/90 border border-neutral-800">
            <div className="flex items-center gap-2">
              <span className="text-xs text-neutral-400 font-light">قیمت:</span>
              <span className="text-sm font-normal text-amber-300 tracking-tight">
            ...
            <div>
              {item.availability === 'available' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[11px] font-medium border border-emerald-500/20 whitespace-nowrap">
                  <CheckCircle2 className="w-3 h-3" />
                  موجود
                </span>
              )}
              {item.availability === 'low_stock' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 text-[11px] font-medium border border-amber-500/20 whitespace-nowrap">
                  <AlertCircle className="w-3 h-3" />
                  تعداد محدود
                </span>
              )}
              {item.availability === 'sold_out' && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-400 text-[11px] font-medium border border-rose-500/20 whitespace-nowrap">
          ...
          {!isSoldOut && (
            <div className="p-2.5 sm:p-3 rounded-xl bg-neutral-950 border border-neutral-800 flex items-center justify-between gap-3 shadow-inner">
              <span className="text-xs text-neutral-300 font-light whitespace-nowrap">
              ...
              {currentQty === 0 ? (
                <button
                  ...
                  className="min-h-[42px] px-4 py-2 rounded-xl text-xs font-semibold text-neutral-950 flex items-center justify-center gap-1.5 active:scale-95 transition-all shadow cursor-pointer whitespace-nowrap shrink-0"
                  style={{ backgroundColor: accentColor }}
                >
                  <Plus className="w-4 h-4" />
                  <span className="whitespace-nowrap">افزودن به انتخاب‌ها</span>
                </button>
              ) : (
                <div className="flex items-center gap-2 bg-neutral-900 border border-neutral-700/60 rounded-xl p-1 shrink-0">
                  <button
                    onClick={() => decreaseItem(item.id)}
                    className="w-7 h-7 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-neutral-200 flex items-center justify-center active:scale-95 transition-all cursor-pointer"
                    aria-label="کاهش"
                  >
                  ...
                  <button
                    onClick={() => addItem(item.id)}
                    className="w-7 h-7 rounded-lg text-neutral-950 flex items-center justify-center active:scale-95 transition-all font-bold cursor-pointer"
                    style={{ backgroundColor: accentColor }}
                    aria-label="افزایش"
                  >
